...
class CounterAzalea(Player):
    ...
    def preflop_strength_for(cards: list[str]) -> float:
        ...
        if r1 == r2:
            return CounterAzalea.clamp(0.4 + high * 0.04, 0.48, 0.96)
        ...
        return CounterAzalea.clamp(score, 0.23, 0.88)
    ...
    # Tuned to beat Azalea's middling call thresholds: open often, defend tighter,
    # and value-bet harder once the sampled equity is clearly ahead.
    def preflop_move(
        self,
        valid_moves: list[Move],
        min_bet: int,
